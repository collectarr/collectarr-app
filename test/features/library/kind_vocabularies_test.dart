...
void main() {
  group('PR 2: Kind-Owned Vocabularies Verification', () {
    test('Comic vocabularies have built-ins for grading, page quality, key categories, publisher', () {
      ...
      expect(ComicVocabularies.pageQuality.builtIns, contains('Off-White to White'));
      expect(ComicVocabularies.keyCategory.builtIns, contains('1st appearance'));
    ...
    test('Game vocabularies have platform, region, edition, age rating, condition', () {
      ...
      expect(GameVocabularies.region.builtIns, contains('NTSC-U/C (US/Canada)'));
      expect(GameVocabularies.ageRating.builtIns, contains('ESRB: Teen (T)'));
      expect(GameVocabularies.condition.builtIns, contains('Complete in Box (CIB)'));
      expect(GameVocabularies.all.length, 5);
    });

    test('Movie & TV vocabularies have packaging, distributor, screen ratio, audio, subtitles', () {
      expect(MovieVocabularies.packaging.builtIns, contains('Steelbook'));
      expect(MovieVocabularies.distributor.builtIns, contains('Criterion Collection'));
      ...
      expect(TvVocabularies.packaging.builtIns, contains('Complete Series Box Set'));
    });

    test('Anime & Manga vocabularies have demographic, studio, serialization, format', () {
      ...
      expect(MangaVocabularies.serialization.builtIns, contains('Weekly Shonen Jump'));
    ...
    test('Book, Music, and BoardGame vocabularies are properly registered', () {
      expect(BookVocabularies.publisher.builtIns, contains('Penguin Random House'));
      ...
      expect(MusicVocabularies.packaging.builtIns, contains('Standard Jewel Case'));

      expect(BoardGameVocabularies.publisher.builtIns, contains('Fantasy Flight Games'));
      expect(BoardGameVocabularies.category.builtIns, contains('Worker Placement'));
    });

    test('All 9 LibraryTypeConfigs have LibraryKindVocabularyCapability attached with matching definitions', () {
      final kinds = [
      ...
      for (final kind in kinds) {
        ...
        final vocCapability = config!.capabilities.vocabulary;
        expect(vocCapability, isNotNull, reason: 'Vocabulary capability for $kind should be set');
        expect(vocCapability!.definitions, isNotEmpty, reason: 'Definitions for $kind should not be empty');
      }
    });
  });
}
