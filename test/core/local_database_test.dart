...
void main() {
  ...
  test('migrates a v7 cache to v35 without losing existing cache rows',
      () async {
    ...
    await old.customStatement(
      'DROP TABLE ${old.mangaOwnedItemsRows.actualTableName}',
    );
    await old.customStatement(
    ...
    expect(version.data.values.first, 35);
  });
  ...
  test('creates Book, Game, and BoardGame tables when migrating from v12',
    ...
    await old.customStatement(
      'DROP TABLE ${old.gameOwnedItemsRows.actualTableName}',
    );
    await old.customStatement(
      'DROP TABLE ${old.boardGameMediaRows.actualTableName}',
    );
    ...
    expect(await db.select(db.bookOwnedItemsRows).get(), isEmpty);
    expect(await db.select(db.mangaOwnedItemsRows).get(), isEmpty);
    ...
    expect(await db.select(db.gameOwnedItemsRows).get(), isEmpty);
  ...
  test('creates Music tables when migrating from v20', () async {
    ...
    await old.customStatement(
      'DROP TABLE ${old.musicOwnedItemsRows.actualTableName}',
    );
    ...
    expect(await db.select(db.musicOwnedItemsRows).get(), isEmpty);
    expect(
  ...
  test('backfills complete Music owned rows when migrating from v30', () async {
    final dir =
        await Directory.systemTemp.createTemp('collectarr_db_v30_music');
    addTearDown(() => dir.delete(recursive: true));
    final file = File('${dir.path}/cache.sqlite');
    ...
    await old.into(old.ownedItemsCache).insert(
          OwnedItemsCacheCompanion.insert(
            id: 'music-owned-legacy',
            itemId: 'music-legacy',
            kind: const Value('music'),
            condition: const Value('Near Mint'),
            grade: const Value('9.5'),
            personalNotes: const Value('Signed first pressing'),
            quantity: const Value(2),
            detailsJson: Value(jsonEncode({
              'storage_device': 'Vinyl',
              'storage_slot': 'Shelf M1',
              'signed_by': 'Artist',
              'last_cleaned_date': '2026-07-02T00:00:00.000Z',
              'matrix_runouts': [
                {
                  'medium_index': 1,
                  'side': 'A',
                  'runout_text': 'ABC-123 A1',
                },
              ],
            })),
            updatedAt: DateTime.utc(2026, 7, 2),
    ...
    final restored = await db.select(db.musicOwnedItemsRows).getSingle();
    expect(restored.id, 'music-owned-legacy');
    expect(restored.itemId, 'music-legacy');
    expect(restored.condition, 'Near Mint');
    expect(restored.grade, '9.5');
    expect(restored.personalNotes, 'Signed first pressing');
    expect(restored.quantity, 2);
    expect(restored.storageDevice, 'Vinyl');
    expect(restored.storageSlot, 'Shelf M1');
    expect(restored.signedBy, 'Artist');
    expect(restored.lastCleanedDate?.toUtc(), DateTime.utc(2026, 7, 2));
    expect(jsonDecode(restored.matrixRunoutsJson), [
      {
        'medium_index': 1,
        'side': 'A',
        'runout_text': 'ABC-123 A1',
  ...
  test('backfills complete Game owned rows when migrating from v31', () async {
    final dir = await Directory.systemTemp.createTemp('collectarr_db_v31_game');
    addTearDown(() => dir.delete(recursive: true));
    final file = File('${dir.path}/cache.sqlite');
    ...
    await old.into(old.ownedItemsCache).insert(
          OwnedItemsCacheCompanion.insert(
            id: 'game-owned-legacy',
            itemId: 'game-legacy',
            kind: const Value('game'),
            condition: const Value('Very Good'),
            grade: const Value('8.5'),
            personalNotes: const Value('Complete in box'),
            quantity: const Value(2),
            detailsJson: Value(jsonEncode({
              'game_completeness': 'Complete in box',
              'game_has_box': true,
              'game_has_manual': true,
              'game_pricecharting_id': 'pc-legacy',
              'game_core_region': 'NTSC-U',
              'game_value_is_locked': false,
            })),
            updatedAt: DateTime.utc(2026, 7, 2),
          ),
        );
    ...
    final restored = await db.select(db.gameOwnedItemsRows).getSingle();
    expect(restored.id, 'game-owned-legacy');
    expect(restored.itemId, 'game-legacy');
    expect(restored.condition, 'Very Good');
    expect(restored.grade, '8.5');
    expect(restored.personalNotes, 'Complete in box');
    expect(restored.quantity, 2);
    expect(restored.completeness, 'Complete in box');
    expect(restored.hasBox, true);
    expect(restored.hasManual, true);
    expect(restored.priceChartingId, 'pc-legacy');
  ...
  test('backfills complete BoardGame owned rows when migrating from v32',
      () async {
    final dir =
        await Directory.systemTemp.createTemp('collectarr_db_v32_boardgame');
    addTearDown(() => dir.delete(recursive: true));
    final file = File('${dir.path}/cache.sqlite');
    ...
    await old.into(old.ownedItemsCache).insert(
          OwnedItemsCacheCompanion.insert(
            id: 'boardgame-owned-legacy',
            itemId: 'boardgame-legacy',
            kind: const Value('boardgame'),
            condition: const Value('Very Good'),
            grade: const Value('8.5'),
            personalNotes: const Value('Deluxe components'),
            quantity: const Value(2),
            detailsJson: Value(jsonEncode({
              'edition_language': 'English',
              'edition_region': 'US',
              'component_condition': 'Like New',
              'component_completeness': 'Complete',
              'missing_pieces_notes': 'None',
              'is_sleeved': true,
              'has_custom_insert': true,
              'has_painted_miniatures': true,
              'storage_notes': 'Shelf 3',
            })),
            updatedAt: DateTime.utc(2026, 7, 2),
          ),
        );
    await old.customStatement(
      'DROP TABLE ${old.boardGameOwnedItemsRows.actualTableName}',
    );
    ...
    final restored = await db.select(db.boardGameOwnedItemsRows).getSingle();
    expect(restored.id, 'boardgame-owned-legacy');
    expect(restored.itemId, 'boardgame-legacy');
    expect(restored.condition, 'Very Good');
    expect(restored.grade, '8.5');
    expect(restored.personalNotes, 'Deluxe components');
    expect(restored.quantity, 2);
    expect(restored.editionLanguage, 'English');
    expect(restored.editionRegion, 'US');
    expect(restored.componentCondition, 'Like New');
    expect(restored.componentCompleteness, 'Complete');
    expect(restored.missingPiecesNotes, 'None');
    expect(restored.isSleeved, true);
    expect(restored.hasCustomInsert, true);
  ...
  test('backfills complete Manga owned rows when migrating from v34', () async {
    final dir =
        await Directory.systemTemp.createTemp('collectarr_db_v34_manga');
    addTearDown(() => dir.delete(recursive: true));
    final file = File('${dir.path}/cache.sqlite');
    ...
    await old.into(old.ownedItemsCache).insert(
          OwnedItemsCacheCompanion.insert(
            id: 'manga-owned-legacy',
            itemId: 'manga-legacy',
            kind: const Value('manga'),
            anchorType: const Value('edition'),
            editionId: const Value('volume-legacy'),
            condition: const Value('Mint'),
            grade: const Value('9.8'),
            personalNotes: const Value('Complete deluxe volume'),
            quantity: const Value(2),
            detailsJson: Value(jsonEncode({
              'raw_or_slabbed': 'Slabbed',
              'grading_company': 'CGC',
              'grader_notes': 'White pages',
              'label_type': 'Modern',
              'custom_label': 'Signed creator copy',
              'page_quality': 'White pages',
              'certification_number': 'CGC-12345',
              'signed_by': 'Takehiko Inoue',
              'obi_strip_present': true,
              'slipcover_present': true,
              'dust_jacket_present': true,
              'dust_jacket_condition': 'Like new',
              'box_set_outer_condition': 'Very good',
              'inserts_present': true,
              'printing': '1st Print',
              'localized_edition': 'VIZ Media',
            })),
    ...
    await old.customStatement('PRAGMA user_version = 34');
    await old.close();

    final db = LocalDatabase(NativeDatabase(file));
    addTearDown(db.close);

    final restored = await db.select(db.mangaOwnedItemsRows).getSingle();
    expect(restored.id, 'manga-owned-legacy');
    expect(restored.itemId, 'manga-legacy');
    expect(restored.anchorType, 'edition');
    expect(restored.editionId, 'volume-legacy');
    expect(restored.condition, 'Mint');
    expect(restored.grade, '9.8');
    expect(restored.personalNotes, 'Complete deluxe volume');
    expect(restored.quantity, 2);
    expect(restored.rawOrSlabbed, 'Slabbed');
    expect(restored.gradingCompany, 'CGC');
    expect(restored.graderNotes, 'White pages');
    expect(restored.labelType, 'Modern');
    expect(restored.customLabel, 'Signed creator copy');
    expect(restored.pageQuality, 'White pages');
    expect(restored.certificationNumber, 'CGC-12345');
    expect(restored.signedBy, 'Takehiko Inoue');
    expect(restored.obiStripPresent, true);
    expect(restored.slipcoverPresent, true);
    expect(restored.dustJacketPresent, true);
    expect(restored.dustJacketCondition, 'Like new');
    expect(restored.boxSetOuterCondition, 'Very good');
    expect(restored.insertsPresent, true);
    expect(restored.printing, '1st Print');
    expect(restored.localizedEdition, 'VIZ Media');
    final version = await db.customSelect('PRAGMA user_version').getSingle();
    expect(version.data.values.first, 35);
  });

  test('destructively rebuilds a higher-versioned cache to the v35 schema',
      () async {
