...
/// Complete Book-owned copy state. Reading progress is a separate concern.
@immutable
final class BookOwnedItem {
  const BookOwnedItem({
    required this.id,
  ...
  Map<String, dynamic> toJson() => {
        ...
        ...bookOwnedTargetLegacyFields(targetRef),
        'condition': condition,
        'grade': grade,
  ...
  factory BookOwnedItem.fromJson(Map<String, dynamic> json) {
    final rawRef = json['catalog_ref'];
    if (rawRef is! Map) {
      throw const FormatException('BookOwnedItem requires catalog_ref');
    }
    final catalogRef =
        CatalogEntityRef.fromJson(Map<String, dynamic>.from(rawRef));
    if (catalogRef.mediaKind != CatalogMediaKind.book) {
      throw FormatException(
        'Expected book catalog_ref, got ${catalogRef.kind}',
      );
    }
    return BookOwnedItem(
      ...
      details: BookOwnedDetails.fromJson(json),
    );
  }
...
CatalogEntityRef? bookOwnedTargetRefFromLegacy(
  CatalogEntityRef catalogRef, {
  String? anchorType,
  String? editionId,
  String? variantId,
...
Map<String, dynamic> bookOwnedTargetLegacyFields(CatalogEntityRef? targetRef) {
