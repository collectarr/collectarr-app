...
///
/// Game intentionally owns the repeated collection fields and its physical
/// completeness/value semantics. Gameplay tracking remains a separate state.
@immutable
final class GameOwnedItem {
  const GameOwnedItem({
    required this.id,
  ...
  final GameOwnedDetails details;
  ...
  Map<String, dynamic> toJson() => {
        ...
        ...gameOwnedTargetLegacyFields(targetRef),
        'condition': condition,
        'grade': grade,
  ...
  factory GameOwnedItem.fromJson(Map<String, dynamic> json) {
    final rawRef = json['catalog_ref'];
    if (rawRef is! Map) {
      throw const FormatException('GameOwnedItem requires catalog_ref');
    }
    final catalogRef =
        CatalogEntityRef.fromJson(Map<String, dynamic>.from(rawRef));
    if (catalogRef.mediaKind != CatalogMediaKind.game) {
    ...
    return GameOwnedItem(
      ...
      targetRef: gameOwnedTargetRefFromLegacy(
        catalogRef,
...
CatalogEntityRef? gameOwnedTargetRefFromLegacy(
  CatalogEntityRef catalogRef, {
  String? anchorType,
  String? editionId,
  String? variantId,
...
Map<String, dynamic> gameOwnedTargetLegacyFields(CatalogEntityRef? targetRef) {
