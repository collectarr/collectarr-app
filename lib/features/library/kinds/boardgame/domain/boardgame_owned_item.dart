...
/// Complete BoardGame-owned copy state.
///
/// Component completeness and storage are owned by BoardGame. Play sessions
/// are tracking data and deliberately remain outside this aggregate.
@immutable
final class BoardGameOwnedItem {
  const BoardGameOwnedItem({
    required this.id,
  ...
  factory BoardGameOwnedItem.fromJson(Map<String, dynamic> json) {
    final rawRef = json['catalog_ref'];
    if (rawRef is! Map) {
      throw const FormatException('BoardGameOwnedItem requires catalog_ref');
    }
    final catalogRef =
        CatalogEntityRef.fromJson(Map<String, dynamic>.from(rawRef));
    if (catalogRef.mediaKind != CatalogMediaKind.boardgame) {
    ...
    return BoardGameOwnedItem(
      ...
      details: BoardgameOwnedDetails.fromJson(json),
    );
  }
