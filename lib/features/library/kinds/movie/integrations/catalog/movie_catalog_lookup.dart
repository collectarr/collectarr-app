...
final class MovieCatalogLookup implements CatalogKindLookup {
  ...
  CatalogSearchHit _hit(MovieMedia media) {
  ...
  bool _matchesBarcode(MovieMedia media, String normalized) {
    ...
    for (final release in media.releases) {
      ...
      for (final entry in release.media) {
        if (_same(_text(entry.rawPayload['barcode']), normalized)) return true;
      }
    }
    return false;
  }

  String? _itemNumber(MovieMedia media) {
