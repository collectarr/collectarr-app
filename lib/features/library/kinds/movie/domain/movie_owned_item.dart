...
///
/// This model intentionally repeats collection fields that exist on other
/// kinds. Movie owns their meaning and storage; cross-kind code uses only
/// structural refs and summaries.
@immutable
final class MovieOwnedItem {
  const MovieOwnedItem({
    required this.id,
  ...
  factory MovieOwnedItem.fromJson(Map<String, dynamic> json) {
    final rawRef = json['catalog_ref'];
    if (rawRef is! Map) {
      throw const FormatException('MovieOwnedItem requires catalog_ref');
    }
    final catalogRef =
        CatalogEntityRef.fromJson(Map<String, dynamic>.from(rawRef));
    if (catalogRef.mediaKind != CatalogMediaKind.movie) {
    ...
    return MovieOwnedItem(
      ...
      details: MovieOwnedDetails.fromJson(json),
    );
  }
