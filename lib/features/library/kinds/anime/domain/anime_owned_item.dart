...
///
/// The model intentionally repeats collection fields that exist on other
/// kinds. Anime owns their meaning and storage; cross-kind code uses only
/// structural refs and summaries. Watch progress remains in AnimeTracking.
@immutable
final class AnimeOwnedItem {
  const AnimeOwnedItem({
    required this.id,
  ...
  Map<String, dynamic> toJson() => {
        ...
        ...animeOwnedTargetLegacyFields(targetRef),
        'condition': condition,
        'grade': grade,
  ...
  factory AnimeOwnedItem.fromJson(Map<String, dynamic> json) {
    final rawRef = json['catalog_ref'];
    if (rawRef is! Map) {
      throw const FormatException('AnimeOwnedItem requires catalog_ref');
    }
    final catalogRef =
        CatalogEntityRef.fromJson(Map<String, dynamic>.from(rawRef));
    if (catalogRef.mediaKind != CatalogMediaKind.anime) {
    ...
    return AnimeOwnedItem(
      ...
      targetRef: animeOwnedTargetRefFromLegacy(
        catalogRef,
      ...
      details: AnimeOwnedDetails.fromJson(json),
    );
  }
...
CatalogEntityRef? animeOwnedTargetRefFromLegacy(
  CatalogEntityRef catalogRef, {
  String? anchorType,
  String? editionId,
  String? variantId,
...
Map<String, dynamic> animeOwnedTargetLegacyFields(CatalogEntityRef? targetRef) {
