...
class AnimeEditDraft extends KindEditDraft {
  AnimeEditDraft({
...
KindEditDraft createAnimeEditDraft({
  ...
  final anime = ownedItem?.animeDetails;
  return AnimeEditDraft(
    featuresController: textControllers.create(text: anime?.features ?? ''),
    boxSetNameController: textControllers.create(text: anime?.boxSetName ?? ''),
    regionController: textControllers.create(text: anime?.region ?? ''),
    packagingController: textControllers.create(text: anime?.packaging ?? ''),
    distributorController:
        textControllers.create(text: anime?.distributor ?? ''),
    ...
    hdrFormats: List<String>.from(anime?.hdrFormats ?? const <String>[]),
  );
}
