...
///
/// Collection fields are deliberately owned by TV even where their names
/// match another kind. Episode progress and watch sessions remain in the
/// separate TV tracking tables.
@immutable
final class TvOwnedItem {
  const TvOwnedItem({
    required this.id,
  ...
  Map<String, dynamic> toJson() => {
        ...
        ...tvOwnedTargetLegacyFields(targetRef),
        'condition': condition,
        'grade': grade,
  ...
  factory TvOwnedItem.fromJson(Map<String, dynamic> json) {
    final rawRef = json['catalog_ref'];
    if (rawRef is! Map) {
      throw const FormatException('TvOwnedItem requires catalog_ref');
    }
    final catalogRef =
    ...
    return TvOwnedItem(
      ...
      details: TvOwnedDetails.fromJson(json),
    );
  }
...
CatalogEntityRef? tvOwnedTargetRefFromLegacy(
  CatalogEntityRef catalogRef, {
  String? anchorType,
  String? editionId,
  String? variantId,
...
Map<String, dynamic> tvOwnedTargetLegacyFields(CatalogEntityRef? targetRef) {
