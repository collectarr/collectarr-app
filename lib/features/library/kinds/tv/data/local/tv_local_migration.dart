...
Future<void> migrateTvOwnedItems(LocalDatabase db) async {
  ...
  for (final row in legacyRows) {
    ...
    final item = TvOwnedItem(
      ...
      details: TvOwnedDetails.fromJson(_decodeDetails(row.detailsJson)),
    );
    await db
        .into(db.tvOwnedItemsRows)
...
Map<String, dynamic> _decodeDetails(String? raw) {
  ...
  try {
    ...
    // Malformed legacy details are represented by TV defaults.
  }
  return const {};
}
