...
Future<LibraryAddProviderCandidatePreview?> loadTvProviderCandidatePreview(
  ...
  if (candidate is! TvProviderCandidate) return null;
  ...
  return LibraryAddProviderCandidatePreview(
    candidate: candidate,
    preview: providerPreviewFromTvEnvelope(envelope),
  );
}
...
CatalogSeriesDetailsDto? _tvSeries(Map<String, dynamic> payload) {
  ...
  return CatalogSeriesDetailsDto(
    ...
    seasonNumber: _tvInt(payload['season_number']),
    episodeNumber: _tvInt(payload['episode_number']),
  );
}

int? _tvInt(Object? value) =>
    value is num ? value.toInt() : int.tryParse(value?.toString() ?? '');

Future<List<TvProviderCandidate>> searchTvProviderCandidates(
  ProviderConnector provider, {
  ...
  return [
    for (final result in results)
      if (result.providerItemId.trim().isNotEmpty && result.kind == kind)
        TvProviderCandidate.fromSearchResult(result, provider: provider.name),
  ];
}

sealed class TvProviderCandidate extends ProviderSearchCandidateBase {
  const TvProviderCandidate({
  ...
  factory TvProviderCandidate.fromSearchResult(
...
final class TvSeriesCandidate extends TvProviderCandidate {
  const TvSeriesCandidate({
    required super.provider,
    required super.providerItemId,
    required super.title,
    super.summary,
