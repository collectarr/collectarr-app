...
//
// All cross-kind operations below dispatch into a concrete kind repository
// and return only structural refs/summaries at the host boundary.
...
/// Explicit summary readers for mixed/global hosts.
final Map<CatalogMediaKind, LibraryOwnedSummaryReader>
    libraryOwnedSummaryReadersByKind = {
...
({JsonMap payload, bool isDeleted}) collectarrTypedOwnedItemSyncPayload(
    CatalogMediaKind kind, Object item) {
  if (kind == CatalogMediaKind.anime) {
    if (item is! AnimeOwnedItem)
      throw ArgumentError.value(
          item, 'item', 'Expected AnimeOwnedItem for anime');
    final payload = JsonMap.from(item.toJson());
