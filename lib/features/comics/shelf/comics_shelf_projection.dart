  ...
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is ComicsShelfProjectionRequest &&
            ...
            identical(other.customFieldValuesByItem, customFieldValuesByItem);
  }
...
class ComicsFilterOptions {
  ...
  factory ComicsFilterOptions.fromEntries(List<ShelfEntry> entries) {
    ...
    for (final entry in entries) {
      _addFilterOption(series, entry.catalogItem?.title);
    ...
    return ComicsFilterOptions(
      series: _sortedFilterOptions(series),
  ...
  return [
    for (final entry in entries)
      if (_matchesComicsKind(entry) &&
          ...
          _matchesValueFilter(entry.catalogItem?.title, filters.series) &&
          ...
          (normalized.isEmpty ||
              _matchesEntryQuery(
                  entry, normalized, customFieldValuesByItem)))
        entry,
  ];
}
...
bool _matchesEntryQuery(
  ...
  if (item != null &&
      (item.title.toLowerCase().contains(query) ||
          ...
          (formatNullableComicDate(item.releaseDate)?.contains(query) ??
              false) ||
