...
import 'package:collectarr_app/features/comics/inspector/comics_personal_details_editor.dart';
import 'package:collectarr_app/features/comics/inspector/comics_rich_metadata_inspector.dart';
import 'package:collectarr_app/features/comics/inspector/metadata_correction_dialog.dart';
...
class ComicInspector extends ConsumerWidget {
  ...
  static final conditions = comicsLibraryConfig.conditions;

  static final grades = comicsLibraryConfig.grades;
  ...
  Future<void> _showEditDialog(
    ...
    for (final edit in selection.itemImageEdits) {
      if (edit.deleted) {
  ...
  Future<void> _updateCollection(
    ...
    await ref.read(collectionMutationsProvider).updateItem(
          ...
          sellPriceCents: ownedItem.sellPriceCents,
          soldTo: ownedItem.soldTo,
        );
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
...
class _EmptyInspector extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const LibraryEmptyInspector(
