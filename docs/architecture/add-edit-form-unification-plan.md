...
- Board Game Add and its Media and Edition Edit schemas now share `BoardGameCatalogFormValues`, kind-owned field specs, and adapters. Manual Add now uses the schema renderer instead of direct controller-backed fields, exposing the typed Work and Edition values through the same definitions used by Edit. The old controller-backed catalog drafts were removed. The legacy generic Board Game Work editor remains separate and still needs a field-by-field migration decision.
...
- The older generic Work edit sessions for Movie, TV, Anime, Comic, Manga, Book, Game, and Board Game remain outside the shared typed field definitions. Their overlapping catalog fields still need a scope-by-scope migration; preserve their kind-specific relations, custom fields, images, and links while doing so.
