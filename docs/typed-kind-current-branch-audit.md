...
HEAD: `4fa57578` (`refactor(activity): compose universal event contributors`)
...
| 58 | Generic calendar becomes event host only | **PARTIAL** | Release projection now leaves the host through registered contributors for all 9 kinds; generic watch fallback remains, while owned lifecycle and loan events now also leave the host through universal contributors. |
| 59 | Kind calendar contributors | **PARTIAL** | All 9 kinds have registered release contributors, and TV/Anime additionally own episode projection; they now implement the shared structural calendar contributor contract, while the full contract matrix remains. |
| 60 | Universal calendar contributors | **PARTIAL** | Calendar now composes `OwnedItemCalendarContributor` and `LoanCalendarContributor` through a shared projection contract; global reminders and a reusable full contract test remain. |
...
| 68 | Activity as projected events | **PARTIAL** | Activity now composes universal Owned/tracking/wishlist/loan lifecycle projections through a structural contributor contract and dispatches TV/Anime episode details through kind-owned contributors; generic watch fallback and the remaining host boundary still need cleanup. |
