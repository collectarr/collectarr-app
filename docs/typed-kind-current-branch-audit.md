...
HEAD: `568ec5ca` (`refactor(tracking): delegate entry sync parsing to kind codecs`)
...
- `tool/check_library_kind_boundaries.dart`: whole-repository baseline currently reports 532 AST architecture violations; its 391 complexity warnings are informational.
...
| 71 | Define genuinely universal tracking infrastructure | **PARTIAL** | Generic `MediaTrackingProfile` now contains only structural options, normalization, and presentation capability; tracking-unit, TV/Anime tracking-entry, and TV/Anime watch-session storage/sync/parsing are orchestration-only through codec contracts, while generic TrackingEntry/WatchSession compatibility and universal lifecycle mechanics still remain. |
...
| 73 | Move watch/custom episode storage | **PARTIAL** | Custom-episode and watch-session persistence, sorting, grouping, sync payload mapping, and remote reconstruction now dispatch through kind-owned TV/Anime codecs, TV episode completion mutations are TV-owned, and the old video/reading/comic tracking-unit union is removed; generic WatchSession storage/model compatibility remains. |
...
|---|---|---|
| Full `lib/**` semantic checker baseline | FAIL | Current checker exits 1 with 532 AST violations; its 391 complexity warnings are informational and the migration allowlist remains explicit/shrinkable. |
