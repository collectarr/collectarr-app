...
HEAD: `852a9e23` (`refactor(tracking): delegate entry coordinates to kind codecs`)
...
- `git diff --name-only main..HEAD`: current branch includes the seed quality guard, Comic export/CSV boundaries, Manga Shelf hierarchy ownership, de-shared video ownership details, explicit personal-list import boundaries, kind-owned Activity projections, kind-owned Admin proposal fields, concrete tracking profiles for every registered kind, kind-owned concrete tracking-unit models and persistence codecs, kind-owned TV/Anime tracking-entry coordinate codecs, kind-owned TV/Anime custom-episode/watch-session codecs, and TV-owned episode completion mutations.
...
| 71 | Define genuinely universal tracking infrastructure | **PARTIAL** | Generic `MediaTrackingProfile` now contains only structural options, normalization, and presentation capability; tracking-unit and TV/Anime tracking-entry storage are orchestration-only through codec contracts, while generic TrackingEntry compatibility and universal lifecycle mechanics still remain. |
| 72 | Full kind-owned tracking state | **PARTIAL** | All 9 registered kinds now own their concrete tracking vocabulary/profile; TV/Anime/Book/Manga/Comic also own concrete tracking-unit models and coordinate persistence/projection codecs, and TV/Anime own tracking-entry coordinate codecs, while typed tracking state/storage is still incomplete for the remaining kinds and generic TrackingEntry remains. |
| 73 | Move watch/custom episode storage | **PARTIAL** | Custom-episode and watch-session persistence, sorting, and grouping now dispatch through kind-owned TV/Anime codecs, TV episode completion mutations are TV-owned, and the old video/reading/comic tracking-unit union is removed; generic WatchSession storage/model compatibility remains. |
...
| 110 | Cross-kind dependency enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 532 violations and still has migration allowlists. |
| 111 | Provider dependency enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 532 violations and still has migration allowlists. |
| 112 | Core DTO ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 532 violations and still has migration allowlists. |
| 113 | Type-erasure enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 532 violations and still has migration allowlists. |
| 114 | Semantic action enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 532 violations and still has migration allowlists. |
| 115 | DB ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 532 violations and still has migration allowlists. |
| 116 | Declarative Add/Edit ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 532 violations and still has migration allowlists. |
...
| 121 | Hierarchy/tracking deletions | **PARTIAL** | Delete-only work has removed multiple legacy surfaces, including the old tracking-unit union; generic tracking-entry coordinate columns/code paths are now delegated to kind codecs, while catalog/Owned/edit/hierarchy and generic TrackingEntry compatibility remains. |
...
|---|---|---|
| Full `lib/**` semantic checker baseline | FAIL | Current checker exits 1 with 532 AST violations; its 392 complexity warnings are informational and the migration allowlist remains explicit/shrinkable. |
