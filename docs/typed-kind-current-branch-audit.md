...
HEAD: `eb76a377` (`refactor(tracking): move episode editor into owning kinds`)
...
- `tool/check_library_kind_boundaries.dart`: whole-repository baseline currently reports 500 AST architecture violations; its 396 complexity warnings are informational.
...
| 110 | Cross-kind dependency enforcement | **PARTIAL** | Game no longer imports Music and the TV episode mutation provider now lives under TV; generic Library → TV detail/edit imports and other migration allowlists remain, while the checker reports 500 violations. |
| 111 | Provider dependency enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 500 violations and still has migration allowlists. |
| 112 | Core DTO ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 500 violations and still has migration allowlists. |
| 113 | Type-erasure enforcement | **PARTIAL** | Catalog persistence now dispatches through typed kind codecs and catalog snapshot mutation inputs are typed; broader `CatalogItem` and metadata compatibility bridges remain, while the checker reports 500 violations. |
| 114 | Semantic action enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 500 violations and still has migration allowlists. |
| 115 | DB ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 500 violations and still has migration allowlists. |
| 116 | Declarative Add/Edit ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 500 violations and still has migration allowlists. |
...
|---|---|---|
| Full `lib/**` semantic checker baseline | FAIL | Current checker exits 1 with 500 AST violations; its 396 complexity warnings are informational and the migration allowlist remains explicit/shrinkable. |
