...
HEAD: `5a814ede` (`refactor(admin): dispatch kind proposal fields`)
...
- `git diff --name-only main..HEAD`: current branch includes the seed quality guard, Comic export/CSV boundaries, Manga Shelf hierarchy ownership, de-shared video ownership details, explicit personal-list import boundaries, and kind-owned Activity projections.
- `tool/check_library_kind_boundaries.dart`: whole-repository baseline currently reports 538 AST architecture violations; its 392 complexity warnings are informational.
...
- The generic import runner is callback-only orchestration; personal import matching/application is supplied by the host, and Activity watch-session projections dispatch TV/Anime episode semantics through kind-owned contributors with a generic coordinate-free fallback.
...
| 69 | Admin kind-specific screens/actions | **PARTIAL** | Proposal editing now dispatches Game platform and Music track fields through explicit kind-owned contributors; the canonical correction dialog and remaining proposal fields still need migration. |
...
| 110 | Cross-kind dependency enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 538 violations and still has migration allowlists. |
| 111 | Provider dependency enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 538 violations and still has migration allowlists. |
| 112 | Core DTO ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 538 violations and still has migration allowlists. |
| 113 | Type-erasure enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 538 violations and still has migration allowlists. |
| 114 | Semantic action enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 538 violations and still has migration allowlists. |
| 115 | DB ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 538 violations and still has migration allowlists. |
| 116 | Declarative Add/Edit ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 538 violations and still has migration allowlists. |
...
|---|---|---|
| Full `lib/**` semantic checker baseline | FAIL | Current checker exits 1 with 538 AST violations; its 392 complexity warnings are informational and the migration allowlist remains explicit/shrinkable. |
