...
HEAD: `94d9eaab` (`refactor(collection): narrow owned tracking sync`)
...
Next active implementation: move the remaining `addLocalOnlyTrackingEntry` episodic arguments behind the TV import contribution while preserving generic timestamps, history, and sync mechanics.
