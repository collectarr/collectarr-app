...
HEAD: `fa1a0ec1` (`test(architecture): distinguish structural event switches`)
...
- The generic import runner is callback-only orchestration; personal import matching/application is supplied by the host, and Activity now composes universal Owned/tracking/wishlist/loan/watch lifecycle projections separately from kind-owned TV/Anime episode contributors, with a generic coordinate-free fallback. Concrete tracking vocabularies now live under each owning kind; Manga no longer reuses Comic's profile, generic tracking keeps only structural profile behavior, tracking-entry coordinate persistence/projection and sync serialization/parsing are dispatched through explicit codecs for all 9 kinds, TV/Anime additionally own episodic coordinate codecs, tracking-unit coordinate persistence/projection is dispatched through explicit kind-owned codecs and concrete TV/Anime/Book/Manga/Comic unit models, custom-episode/watch-session persistence is dispatched through TV/Anime codecs, watch-session sync payload parsing/serialization is dispatched through TV/Anime codecs with a coordinate-free common fallback, TV episode completion mutations are TV-owned, the generic tracking mutation API now accepts kind customization rather than episode-ratings payload fields, generic personal field ownership no longer advertises episodic coordinates, the generic inspector now renders only structural tracking controls, TV/Anime own their episode coordinate editor extensions, Movie no longer advertises episodic capability, and the generic Owned sync path no longer accepts new episodic coordinates.
...
- Seed coverage: 15 catalog entries for every kind, with all nine typed catalog graph families enriched and persisted (including Comic issues, Manga chapters, Book/Game/BoardGame/Movie releases, TV/Anime episodes, and Music media/tracks). All nine kind-owned ownership detail tables are populated; TV/Anime typed episodic tracking is seeded from the persisted graph; and Comic/Manga/Book/TV/Anime typed tracking units are seeded through the registered coordinate codecs. The dev test and CLI enforce minimum typed graph/owned/tracking/tracking-unit/auxiliary counts and reseed idempotency.
...
| 58 | Generic calendar becomes event host only | **DONE** | The provider now only loads titles/data, dispatches registered kind and universal contributors, and sorts projected events; release/watch semantic extraction no longer lives in the host. |
| 59 | Kind calendar contributors | **DONE** | All 9 kinds have registered release contributors, TV/Anime additionally own episode projection, all implement the shared structural calendar contributor contract, and the full contract matrix passes. |
| 60 | Universal calendar contributors | **PARTIAL** | Calendar composes owned lifecycle, loan, and generic watch projections through the shared contract; global reminders and their contributor coverage remain. |
...
| 68 | Activity as projected events | **DONE** | Activity composes universal Owned/tracking/wishlist/loan/watch lifecycle projections through a structural contributor contract and dispatches TV/Anime episode details through kind-owned contributors; the generic host no longer constructs watch events or inspects hierarchy coordinates. |
...
| 71 | Define genuinely universal tracking infrastructure | **PARTIAL** | Generic `MediaTrackingProfile` now contains only structural options, normalization, and presentation capability; tracking-unit, TV/Anime tracking-entry, TV/Anime watch-session, and TV/Anime custom-episode storage/sync/parsing are orchestration-only through kind codecs, while generic TrackingEntry compatibility and universal lifecycle mechanics still remain. Generic TrackingEntry sync fallback no longer parses episodic fields, the mutation API accepts a structural kind customizer instead of episode-ratings payload fields, generic personal field ownership no longer advertises episodic coordinates, and the generic inspector delegates episodic editing to kind-owned TV/Anime extensions. Movie no longer advertises the removed episodic capability. The generic Owned sync path no longer accepts new episodic coordinates, and the remaining TV season coordinate for local TMDb import now enters through a registry-composed TV contribution. Episodic selection state and TV metadata application were removed from the shared video edit controller and moved into the TV/Anime drafts. Core `WatchSession.fromJson` and `CustomEpisode.fromJson` factories were deleted; unsupported remote session/episode kinds now fail explicitly instead of being reconstructed by a generic semantic parser. Seed fixtures exercise typed units plus TV/Anime typed repositories. |
| 72 | Full kind-owned tracking state | **PARTIAL** | All 9 registered kinds now own their concrete tracking vocabulary/profile and tracking-entry codec; TV/Anime/Book/Manga/Comic also own concrete tracking-unit models and coordinate persistence/projection codecs, and TV/Anime own episodic inspector/editor extensions. Dev fixtures persist TV episode progress, Anime episode tracking, and 15/15/15/30/30 typed coordinate units, while typed tracking state/storage is still incomplete for the remaining kinds and generic TrackingEntry remains. |
| 73 | Move watch/custom episode storage | **PARTIAL** | Custom-episode and watch-session persistence, sorting, grouping, sync payload mapping, and remote reconstruction now dispatch through kind-owned TV/Anime codecs, TV episode completion mutations are TV-owned, and the old video/reading/comic tracking-unit union is removed. Core generic reconstruction factories are deleted; `WatchSession`/`CustomEpisode` remain only as structural persistence projections used by codecs, and unknown sync kinds fail explicitly. The inspector's episode coordinate controls are now contributed by TV/Anime rather than the generic tracking editor. |
...
| 75 | De-share Video | **PARTIAL** | Movie, TV, and Anime now own independent physical-copy detail models; generic Collection/Detail/Inspector code no longer reads shared video semantics, and shared video edit no longer owns episodic selection state. Shared video metadata/edit/catalog/tracking compatibility remains. |
...
| 111 | Provider dependency enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 494 violations and still has migration allowlists. |
| 112 | Core DTO ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 494 violations and still has migration allowlists. |
| 113 | Type-erasure enforcement | **PARTIAL** | Catalog persistence now dispatches through typed kind codecs and catalog snapshot mutation inputs are typed; broader `CatalogItem` and metadata compatibility bridges remain, while the checker reports 494 violations. |
| 114 | Semantic action enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 494 violations and still has migration allowlists. |
| 115 | DB ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 494 violations and still has migration allowlists. |
| 116 | Declarative Add/Edit ownership enforcement | **PARTIAL** | Several architecture guards exist, but the current whole-repository checker reports 494 violations and still has migration allowlists. |
...
| 121 | Hierarchy/tracking deletions | **PARTIAL** | Delete-only work has removed multiple legacy surfaces, including the old tracking-unit union and generic inspector episode controls; generic tracking-entry coordinate columns, storage, and sync payload code paths are now delegated to kind codecs, while catalog/Owned/edit/hierarchy and generic TrackingEntry compatibility remains. |
...
|---|---|---|
| Full `lib/**` semantic checker baseline | FAIL | Current checker exits 1 with 494 AST violations; its 396 complexity warnings are informational and the migration allowlist remains explicit/shrinkable. |
...
PR0 is complete as a refreshed rebaseline. The branch has substantial prior typed-kind work and the seed fixtures are coherent, enriched, and validated, but it is not yet compliant with the new definition of done. PR1 is implemented as a baseline: the checker applies boundary rules across the relevant production `lib/**` surface, skips only explicit generated/composition roots, and keeps migration allowlists visible. PR2-6 added reusable contracts, strict Core field adoption, an explicit nine-kind manifest, an owned-edit registration boundary, and typed read repositories. PR11-13 added structural action, import/export, and generic menu contracts. PR16 and PR63 made owned read projections and loan references explicit. PR64-70 consolidated generic infrastructure while moving vocabularies, personal imports, Activity lifecycle projections, admin proposal fields, and settings semantics toward their owning boundaries. The seed entry point now validates catalog/owned/tracking coverage and quality before writes, supplements every kind-owned vocabulary without erasing catalog-derived values, and enriches all nine typed catalog graphs before persistence. Comic has the most complete typed vertical slice; Manga hierarchy, de-shared video ownership details, kind-owned calendar/barcode contributors, typed tracking-entry codecs for all 9 kinds, and typed TV/Anime tracking persistence plus inspector editing have also landed. Calendar now composes universal owned/loan/watch projections and TV TMDb season tracking is routed through its kind contribution. Activity now composes universal Owned/tracking/wishlist/loan projections behind a structural contributor contract. Core session/episode reconstruction now requires an owning kind codec, closing another generic semantic parser boundary. Remaining compatibility edges include generic catalog/owned/edit/tracking surfaces and migration allowlists. The current checker baseline is 494 AST violations with 396 informational complexity warnings and must shrink in subsequent migrations.

## Recommended next PR

Latest verification after typed seed coverage, AniList boundary cleanup, legacy mapper deletion, TV hierarchy migration, TV personal-import ownership, reset-script cleanup, universal Calendar contributors, generic Calendar watch projection, TV season import ownership, universal Activity contributors, generic Activity watch projection, narrowed generic tracking mutation payloads, episodic field ownership cleanup, strict kind-owned session reconstruction, all-kind tracking-entry codecs, TV/Anime inspector extensions, and shared-video episodic-state extraction: the checker reports 494 AST violations and 396 informational complexity warnings.

Since the previous audit text, tracking-entry sync reconstruction/serialization now has an explicit codec for every registered kind; TV/Anime tracking-entry sync reconstruction and watch-session sync reconstruction/serialization remain behind their kind codecs, and the common fallback now serializes only lifecycle fields. Episodic inspector editing also moved behind explicit TV/Anime inspector capabilities, leaving the generic inspector with progress and universal tracking controls only.
...
TV personal episode import mapping now lives under the TV integration boundary; the generic personal-list host remains responsible only for file parsing/orchestration. The local TMDb season tracking coordinate now follows the same kind-owned contribution path through the registry. Calendar projects owned lifecycle and loan events through universal contributors, while the host only merges projected events. The local reset script also scopes process cleanup to the current workspace and removes all SQLite journal sidecars, preventing unrelated Dart processes or stale WAL files from affecting development verification.

Current branch recommendation: `PR72 continuation - expand kind-owned tracking state`.
Next active implementation: audit the remaining generic TrackingEntry compatibility fields and move the next concrete tracking projection behind an owning-kind codec, preserving universal timestamps, history, and sync mechanics.
